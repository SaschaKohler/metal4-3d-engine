...
Mesh* Mesh::loadGLB(MTL::Device* device, const std::string& path) {
    ...
    // Try path as-is first, then relative to executable
    ...
    if (result != cgltf_result_success) {
        // Retry with executable-relative path
        std::string exePath = executableDir() + "/" + path;
        result = cgltf_load_buffers(&options, data, exePath.c_str());
    }
    if (result != cgltf_result_success) {
    ...
    mesh->m_vertexBuffer = device->newBuffer(
        vertices.data(),
        vertices.size() * sizeof(Vertex),
        MTL::ResourceStorageModeShared);

    mesh->m_indexBuffer = device->newBuffer(
        indices.data(),
    ...
    printf("[Mesh] Loaded %s — %zu vertices, %zu indices\n",
           path.c_str(), vertexCount, indices.size());
